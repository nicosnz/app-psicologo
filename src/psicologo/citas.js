...
function registrarCitaAdmin(c, color, calAdmin) {
...
function registrarCitaCliente(c, color, calAdmin, calCliente) {
  const paciente = c.pacientes || null;
  const citaCliente = {
  ...
  const fcEventCliente = calCliente.addEvent({
    title: c.titulo, start: c.inicio, end: c.fin,
    backgroundColor: color, borderColor: color,
  ...
  const adminEntry = {
    ...
    todoElDia: false, esSolicitudCliente: true, citaCliente, fcEventCliente,
  };
  citasAdmin.push(adminEntry);

  const fcEventAdmin = calAdmin.addEvent({
    title: `${prefijo} ${c.titulo}`, start: c.inicio, end: c.fin,
    backgroundColor: color, borderColor: color,
...
export async function cargarCitas(calAdmin, calCliente) {
  ...
  citas.forEach(c => {
    ...
    if (c.origen === 'admin') registrarCitaAdmin(c, color, calAdmin);
    else registrarCitaCliente(c, color, calAdmin, calCliente);
  });
